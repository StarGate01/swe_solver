...
\setbeamertemplate{footline}
{
	...
	\begin{document}
		...
		\begin{frame}{HPC: Performance analysis (HPC $\leftrightarrow$ PC)}
			...
			xlabel={cpu time [s]},
			xlabel style={at={(axis description cs:0.5,-0.22)}, anchor=south},
			...
			\end{tikzpicture}
			\newline
			{\tiny GNU, all optimizations, release}\\\vspace{15pt}
			Result
			\begin{itemize}
				\item Vgl. heavy 
			\end{itemize}
		...
		\begin{frame}{GCC $\leftrightarrow$ Intel}
			...
			\addplot coordinates {(13.04,light) (1061.26,heavy)};
			\addplot coordinates {(9.55,light) (1777.74,heavy)};
			\legend{gnu,intel}
			\end{axis}
			\end{tikzpicture}
			\newline
