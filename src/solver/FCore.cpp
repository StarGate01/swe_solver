...
struct fresult FCore::compute(qvector ql, qvector qr)
{
    ...
    if(res.lambda_1 < 0)            /**< In case lambda_1 is negative, assign AdQ appropriatelly*/
    {
        res.adq_negative.x += alpha.x * r1.x;
        res.adq_negative.y += alpha.x * r1.y;
    }
    else if(res.lambda_1 > 0)       /**< In case lambda_1 is positive, assign AdQ appropriatelly*/
    {
        res.adq_positive.x += alpha.x * r1.x;
        res.adq_positive.y += alpha.x * r1.y;
    }
 
    if(res.lambda_2 < 0)            /**< In case lambda_2 is negative, assign AdQ appropriatelly*/
    {
        res.adq_negative.x += alpha.x * r2.x;
        res.adq_negative.y += alpha.x * r2.y;
    }
    else if(res.lambda_2 > 0)       /**< In case lambda_1 is positive, assign AdQ appropriatelly*/
    {
        res.adq_positive.x += alpha.x * r2.x;
        res.adq_positive.y += alpha.x * r2.y;  
    }
             
    if(res.lambda_1 < 0 && res.lambda_2) /**< Treat special cases*/
        res.lambda_2 = 0;
